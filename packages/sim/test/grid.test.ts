...
describe("MapGrid.tryMove", () => {
  it("lets road players walk on road", () => {
    expect(grid.tryMove({ x: 1, y: 6, layer: "road" }, DIRS.east)).toEqual({ x: 2, y: 6, layer: "road" });
  });

  it("blocks road players from walking onto a wall", () => {
    expect(grid.tryMove({ x: 3, y: 6, layer: "road" }, DIRS.north)).toBeNull();
  });

  it("blocks wallTop players from stepping down onto road", () => {
    expect(grid.tryMove({ x: 3, y: 5, layer: "wallTop" }, DIRS.south)).toBeNull();
  });

  it("changes layer only when leaving stairs", () => {
    const ontoStairs = grid.tryMove({ x: 1, y: 5, layer: "road" }, DIRS.east);
    expect(ontoStairs).toEqual({ x: 2, y: 5, layer: "road" });
    const ontoWall = grid.tryMove(ontoStairs!, DIRS.east);
    expect(ontoWall).toEqual({ x: 3, y: 5, layer: "wallTop" });
    const backOntoStairs = grid.tryMove(ontoWall!, DIRS.west);
    expect(backOntoStairs).toEqual({ x: 2, y: 5, layer: "wallTop" });
    expect(grid.tryMove(backOntoStairs!, DIRS.west)).toEqual({ x: 1, y: 5, layer: "road" });
  });

  it("lets both layers use a bridge cell without mixing", () => {
    expect(grid.tryMove({ x: 5, y: 4, layer: "wallTop" }, DIRS.north)).toEqual({ x: 5, y: 3, layer: "wallTop" });
    expect(grid.tryMove({ x: 4, y: 3, layer: "road" }, DIRS.east)).toEqual({ x: 5, y: 3, layer: "road" });
    expect(grid.tryMove({ x: 5, y: 3, layer: "road" }, DIRS.north)).toBeNull();
    expect(grid.tryMove({ x: 5, y: 3, layer: "wallTop" }, DIRS.east)).toBeNull();
  });

  it("finds the rise direction of stairs", () => {
    expect(grid.stairsRiseDir(2, 5)).toEqual(DIRS.east);
  });

  it("spawns next to the tower", () => {
    expect(grid.spawnTiles()).toContainEqual({ x: 2, y: 4, layer: "road" });
  });
});
...
describe("validateMap", () => {
  ...
  it("rejects key spawns that are unreachable or too few", () => {
    ...
    const onWall = { ...TINY_MAP, spawns: { ...TINY_MAP.spawns, keys: [{ x: 3, y: 5, layer: "road" as const }] } };
